...
object AllBBReader {
    ...
    private fun fetchChart(root: File, metadata: BBJournalMetadata, theChartMetadata: BBChartMetadata) {
        ...
        while (theSkip <= POSSIBLE_SKIPPED_WEEKS_IN_ROW) {
            ...
            val theChartFile = File(theChartDir,
                    theChartMetadata.prefix + "-" + theFormatDate + ".json")
            if (!theChartFile.exists()) {
                try {
                    val theChartDocument = BBHtmlParser.getChartDocument(metadata, theChartMetadata,
                            theFormatDate)
                    val theHtmlDate = BB.CHART_DATE_FORMAT.format(BBHtmlParser.getChartDate(theChartDocument))
